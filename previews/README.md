...
Component previews for **mythical-ui** — moved here from the mythical-design tokens repo
 because they render this repo's components; the tokens repo now only owns
...
The pane holds mythical-design's **26** cards, including `components-file-explorer`,
`components-timeline`, `product-glyphs`, `layouts-app-shell` and `layouts-settings` — none of
which exist here — plus `tokens.css`, `assets/fonts/` and the logos, which live only in
mythical-design. The two sets are **both live, with different jobs**:
...
→ apply the same wording to the matching `mythical-design/ds/` card, which is what the Design pane
is synced from.
