// packages/react-ui/confirm.test.tsx — the generic ConfirmDialog contract (design note §1b-D21):
...
// imported (not reimplemented) from `@mythicalos/ui-core`.
//
// React twin of packages/preact-ui/confirm.test.tsx — same assertions, `renderToStaticMarkup`
...
describe("ConfirmDialog — render contract", () => {
  test("plain danger confirm: confirm button enabled, Esc/safe-action copy present", () => {
    const html = renderToStaticMarkup(
      <ConfirmDialog title="Delete job?" body="really" confirmLabel="Delete" onConfirm={noop} onCancel={noop} />,
    );
  ...
  test("no mythical lifecycle vocabulary anywhere in the rendered markup", () => {
    const html = renderToStaticMarkup(<ConfirmDialog title="T" body="b" confirmLabel="Do" onConfirm={noop} onCancel={noop} />);
