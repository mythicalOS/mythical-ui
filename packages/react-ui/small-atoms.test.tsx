// packages/react-ui/small-atoms.test.tsx — React twin of packages/preact-ui/small-atoms.test.tsx.
// Same assertions, same core-derived expectations: SaveBar/StatTiles/GitChip must derive 100% of
// their class strings and copy from `@mythicalos/ui-core/logic`, so the two bindings cannot drift.
// (See parity.test.tsx for why side-by-side rendering of both bindings in one process isn't
...
describe("SaveBar", () => {
  test("renders the card's sentence, exactly as ui-core composes it", () => {
    const html = renderToStaticMarkup(
      <SaveBar changed={["Base URL"]} onDiscard={noop} onSave={noop} />,
    );
  ...
  test("pluralizes through ui-core, never locally", () => {
    ...
    const html = renderToStaticMarkup(<SaveBar changed={changed} onDiscard={noop} onSave={noop} />);
  ...
  test("NOT dirty ⇒ renders nothing (the card's 'only when dirty' rule, owned by the atom)", () => {
    expect(renderToStaticMarkup(<SaveBar changed={[]} onDiscard={noop} onSave={noop} />)).toBe("");
    expect(
      renderToStaticMarkup(<SaveBar changed={["", "  "]} onDiscard={noop} onSave={noop} />),
    ).toBe("");
  });

  test("default action copy is the design card's; both are overridable", () => {
    const html = renderToStaticMarkup(<SaveBar changed={["X"]} onDiscard={noop} onSave={noop} />);
  ...
  test("saving ⇒ the primary goes loading + inert, the discard stays live", () => {
    const html = renderToStaticMarkup(
      <SaveBar changed={["X"]} saving onDiscard={noop} onSave={noop} />,
    );
  ...
  test("an extra className is appended alongside the core-derived class", () => {
    const html = renderToStaticMarkup(
      <SaveBar changed={["X"]} className="extra" onDiscard={noop} onSave={noop} />,
    );
...
describe("StatTiles", () => {
  ...
  test("duplicate labels do not collide", () => {
    ...
    const html = renderToStaticMarkup(<StatTiles tiles={dupes} />);
...
describe("GitChip — available arm", () => {
  ...
  test("the card's dirty chip: branch + three flags with core-derived classes", () => {
    const html = renderToStaticMarkup(<GitChip status={dirty} />);
...
describe("GitChip — unavailable arm (the honesty this atom exists for)", () => {
  test("no status ⇒ the neutral note and the unavailable modifier, never a clean row", () => {
    const html = renderToStaticMarkup(<GitChip />);
  ...
  test("a product-supplied reason wins over the defaults and is also the branch-slot title", () => {
    ...
    const html = renderToStaticMarkup(<GitChip unavailableNote={reason} />);
  ...
  test("loading, with no reason, is the in-flight line", () => {
    const html = renderToStaticMarkup(<GitChip loading />);
    expect(html).toContain(GIT_LOADING_NOTE);
  });

  test("a reason outranks loading", () => {
    const html = renderToStaticMarkup(<GitChip loading unavailableNote="No worktree recorded." />);
...
describe("GitChip — stale is flagged on BOTH arms", () => {
  ...
  test("unavailable + stale", () => {
    const html = renderToStaticMarkup(<GitChip unavailableNote="Read failed." stale />);
