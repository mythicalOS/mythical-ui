...
// packages/preact-ui/theme-toggle.test.tsx — render contracts for the theme toggle family:
// ThemeToggle (segmented), ThemeToggleIcon, ThemeToggleSwitch. Every expected class string, glyph
// path and piece of copy is derived by importing the CORE functions and constants, never
// hard-coded, so the binding and @mythicalos/ui-core cannot drift.
...
describe("ThemeToggle — the segmented member", () => {
  test("the root class is themeToggleClass(...), verbatim, across the whole matrix", () => {
    for (const mode of THEME_MODES) {
      expect(renderToString(<ThemeToggle mode={mode} onModeChange={noop} />)).toContain(
        `class="${themeToggleClass(mode)} "`,
      );
      expect(renderToString(<ThemeToggle mode={mode} onModeChange={noop} labelled />)).toContain(
  ...
  test("it is a real radiogroup with a name, holding three real radios", () => {
    const html = renderToString(<ThemeToggle mode="system" onModeChange={noop} />);
  ...
  test("System is FIRST — the card's whole point about it being a real choice", () => {
    const html = renderToString(<ThemeToggle mode="system" onModeChange={noop} labelled />);
  ...
  test("exactly one option is aria-checked, and it is the mode handed in", () => {
    for (const mode of THEME_MODES) {
      const html = renderToString(<ThemeToggle mode={mode} onModeChange={noop} labelled />);
  ...
  test("icon-only options carry a name; labelled ones let the visible word be the name", () => {
    const icons = renderToString(<ThemeToggle mode="system" onModeChange={noop} />);
    for (const m of THEME_MODES) expect(icons).toContain(`aria-label="${THEME_MODE_LABELS[m]}"`);
    const labelled = renderToString(<ThemeToggle mode="system" onModeChange={noop} labelled />);
  ...
  test("the cross-fade classes belong to the ICON member alone", () => {
    ...
    for (const checked of [true, false]) {
      const html = renderToString(<ThemeToggleSwitch checked={checked} onChange={noop} />);
  ...
  test("the passthrough class is appended, never replaced", () => {
    expect(renderToString(<ThemeToggle mode="dark" onModeChange={noop} class="extra" />)).toContain(
  ...
  test("rendering never invokes onModeChange — only a real activation may", () => {
    let calls = 0;
    renderToString(<ThemeToggle mode="light" onModeChange={() => (calls += 1)} />);
    expect(calls).toBe(0);
  });
...
describe("ThemeToggleIcon — the compact member", () => {
  test("the root class is themeIconClass(...), verbatim, in both variants", () => {
    expect(renderToString(<ThemeToggleIcon isDark={false} onToggle={noop} />)).toContain(
      `class="${themeIconClass()} "`,
    );
    expect(renderToString(<ThemeToggleIcon isDark={false} onToggle={noop} bordered />)).toContain(
  ...
  test("it is a named toggle button whose state rides aria-pressed", () => {
    for (const isDark of [true, false]) {
      const html = renderToString(<ThemeToggleIcon isDark={isDark} onToggle={noop} />);
  ...
  test("rendering never invokes onToggle", () => {
    let calls = 0;
    renderToString(<ThemeToggleIcon isDark onToggle={() => (calls += 1)} />);
    expect(calls).toBe(0);
  });
});
...
describe("ThemeToggleSwitch — the settings-row member", () => {
  test("a real checkbox inside a label, with the card's parts", () => {
    const html = renderToString(<ThemeToggleSwitch checked={false} onChange={noop} />);
  ...
  test("the checked state is the input's own, in both directions", () => {
    expect(renderToString(<ThemeToggleSwitch checked onChange={noop} />)).toContain("checked");
    expect(renderToString(<ThemeToggleSwitch checked={false} onChange={noop} />)).not.toContain(
      " checked",
    );
  });
  ...
  test("disabled is ANNOUNCED by the input, not merely painted on the label", () => {
    const html = renderToString(<ThemeToggleSwitch checked={false} onChange={noop} disabled />);
  ...
  test("rendering never invokes onChange", () => {
    let calls = 0;
    renderToString(<ThemeToggleSwitch checked onChange={() => (calls += 1)} />);
    expect(calls).toBe(0);
  });
});
...
describe("the family emits no inline styles (CSP style-src 'self')", () => {
  test("no render of any member carries a style attribute", () => {
    ...
    const renders = [
      ...
      renderToString(<ThemeToggleIcon isDark={false} onToggle={noop} />),
      renderToString(<ThemeToggleSwitch checked onChange={noop}>Dark mode</ThemeToggleSwitch>),
      renderToString(<ThemeToggleSwitch checked={false} onChange={noop} disabled />),
    ];
