// packages/preact-ui/hooks.test.ts — the polling policy's HOOK-LEVEL guard. The pure scheduling math this hook is built from
...
// The scan must be BINDING — raw-substring checks are decoy-prone: a commented-out
...
describe("usePoll routes every response through the epoch guard (source scan — diff r3-F1, comment-stripped per r4-F1)", () => {
  ...
  test("the hook's tick delegates to runPollTick — no un-guarded fetch path remains", () => {
    ...
    expect(body).not.toContain("await fnRef.current()"); // the unguarded direct-apply shape
    // The unguarded promise-chain shapes. The live body invokes `fnRef.current()` ONLY as
...
// second poll chain (the failure shape: an effect that keeps one `timer` variable, whose
// visibilitychange handler clears it and fires an immediate tick on return — but if a tick is
// ALREADY in flight no timer is armed to clear, so a second concurrent tick starts and each chain's own
