// @mythicalos/preact-ui — small shared hooks (polling + interval). A family UI talks plain
// request/response to its service; pages poll on their cadence via `usePoll`. The polling
// policy: per-call interval, ±20% jitter on every delay, exponential ×2 backoff on consecutive failures
...
// (`nextPollDelay`, `makePollEpochGuard`, `runPollTick`, `shouldResetEpoch` — the fire-time
// epoch stamp) lives in `@mythicalos/ui-core`. Only the actual Preact hooks (`usePoll`/`useInterval`, which import `preact/hooks` and
// so can never live in the framework-agnostic core) stay here, wired to the imported pure functions.
...
/**
 * Poll `fn` on the `ms` cadence while `enabled`, under the polling policy: every delay ±20% jittered,
...
export function usePoll<T>(
  ...
  // One persistent guard per call site — the render-time reset below invalidates it
  ...
  if (shouldResetEpoch(useEpoch, epochRef.current, epochKey, enabled)) {
    guard.invalidate(); // FIRST — stale in-flight responses die here, not at deferred cleanup
  ...
  useEffect(() => {
    ...
    const schedule = () => {
      ...
      // Hidden-document pause: while the document is hidden no timer is armed — the
      // visibilitychange handler below refreshes immediately on return.
      if (isHidden()) return;
      timer = setTimeout(() => void tick(), nextPollDelay(ms, failures));
    };

    // Every response routes through runPollTick — stamped with the epoch at fire
    ...
    return () => {
      alive = false;
      stopped = true;
      guard.invalidate(); // teardown supersedes the epoch for legacy (no-epochKey) callers too
