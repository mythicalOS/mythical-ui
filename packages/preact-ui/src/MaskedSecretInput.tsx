...
// @mythicalos/preact-ui — masked secret input with slot chip (ds/components-inputs "secret slots").
...
export function MaskedSecretInput(props: MaskedSecretInputProps) {
  ...
  const chip = props.filled ? <span class="chip">{props.slot}</span> : <span class="chip chip--empty">slot empty</span>;
  ...
  if (props.filled && !editing) {
    return (
      <label class="field">
        ...
        <div class="slot">
          <input class="input mono readonly-input" value={props.masked ?? "••••••••"} readonly />
          {chip}
          <button type="button" class="rep" disabled={props.disabled} onClick={() => setEditing(true)}>
            Replace
          </button>
        </div>
        <div class="help">Readonly mask; Replace re-opens entry. The value never round-trips to the UI.</div>
      </label>
    );
  }
  ...
  return (
    <label class="field">
      ...
      <div class="slot">
        <input
          class="input mono"
          // A replacement credential is a SECRET while typed — masked entry
      ...
      <div class="help">Unconfigured is a valid state — no error until submit.</div>
    </label>
  );
}
