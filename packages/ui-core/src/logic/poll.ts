// @mythicalos/ui-core — the pure, DOM-free polling policy math + fire-time epoch guard. Ported
...
// consecutive failures capped at 60 s (reset on success), and the fire-time epoch stamp
// so a stale in-flight response for an old entity can never repaint a newer one's pane.

/** Polling policy constants: every delay is jittered ±20%; the failure-backoff ladder tops out at 60 s. */
export const POLL_JITTER_RATIO = 0.2;
export const POLL_BACKOFF_CAP_MS = 60_000;

/**
 * The scheduling policy, pure + DOM-free: the next poll delay for a base cadence after
...
/**
 * A MONOTONIC request-epoch guard for usePoll:
 * the render-time epoch reset clears retained data, but Preact runs effect CLEANUP after
...
export interface PollTickIO<T> {
  ...
  isAlive: () => boolean;
  /** Hidden-document pause: a hidden document never fires a read. */
  isHidden: () => boolean;
...
/**
 * One poll tick with fire-time epoch stamping: the epoch token is captured BEFORE
...
export async function runPollTick<T>(io: PollTickIO<T>): Promise<void> {
  ...
  const token = io.guard.current(); // fire-time stamp
  let applied = false;
  try {
