 ...
 * filter being enabled reads `noise off`.
 *
 * Labelling the FILTER instead (filter on ⇒ `noise on`, while noise is in fact HIDDEN) is the
 * wrong reading: in a status line that sits beside `tail`, a reader takes `noise on` to mean
 * noise is being shown. The default would compound it: the filter ships enabled
 * (`terminal_noise_filter: true`), so an untouched pane would open claiming `noise on` while
 * suppressing the init line, tool-reference blobs and skill preambles.
 *
 * The parameter keeps its name because it is the FILTER flag the caller holds; only the word
 * rendered for it differs. `noiseShow()` is unaffected — it maps filter → renderer option
