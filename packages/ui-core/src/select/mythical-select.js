/*! mythical-select v3 — form-associated, native-parity styled <select> for the
    ...
    Native-parity surface:
      ...
      limits:     a MutationObserver reports mutations asynchronously and in
                  ...
                  three engines (probed). These are the residual,
                  non-consumer-plausible gaps.
  ...
  class MythicalSelect extends HTMLElement {
    ...
    constructor() {
      ...
      this.#mo.observe(this, { childList: true, subtree: true, attributes: true,
        ...
        attributeOldValue: true, // lets #relevant drop same-value re-writes (no-op records)
    ...
    /* Records worth re-adopting for. Attribute records on the HOST itself belong
       ...
       EXCEPT a same-value attribute re-write: setAttribute queues a record even when the
       value did not change, and vdom frameworks legitimately re-assert value="…" on every
       render (the value prop is exempt from old-vs-new short-circuits because form DOM state
       can drift). Re-adopting on those no-ops closes an OPEN popup mid-interaction (#adopt's
       close-before-rebuild) — observed as "the dropdown closes itself" on any page that
       re-renders on a poll. A record is a no-op only if its oldValue equals the attribute's
       CURRENT value; in a queued A→B→A sequence the middle record still differs, so a real
       round-trip keeps re-adopting. Requires attributeOldValue: true at the observe site. */
    #relevant(recs) {
      return recs.filter(r => {
        if (r.type !== 'attributes') return true;
        if (r.target === this) return false;
        return r.oldValue !== r.target.getAttribute(r.attributeName);
      });
    }
