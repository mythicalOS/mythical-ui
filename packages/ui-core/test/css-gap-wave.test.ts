// test/css-gap-wave.test.ts — stylesheet coverage for the mockup gap wave: the tone-outlined
...
//
// A separate file (rather than more cases in css.test.ts / css-small-atoms.test.ts) purely so
// this wave adds no contended edits to files other in-flight work also touches. The tone-line
// block's own contract lives in test/css-button-tone-line.test.ts.
...
describe("callout family + the ? opener", () => {
  ...
  test("the opener is round BY RULING-PENDING EXCEPTION, and announces itself from the binding", () => {
    ...
    // rule-10 scoped exception pending the maintainer's design-book ruling. Pinned so a change
